...
public final class WebSocketConfigurator extends Configurator {
	...
	private static Log logger = LogFactory.getLog(WebSocketConfigurator.class);
	...
	private String getBeanNameByType(final WebApplicationContext wac, final Class<?> endpointClass) {
		...
		if (!beanNamesByType.containsKey(endpointClass)) {
			...
			if (names.length == 1) {
				...
				if (names.length > 1) {
					final String message = "Found multiple @ServerEndpoint's of type " + endpointClass + ", names=" + names;
					logger.error(message);
					throw new IllegalStateException(message);
				}
			}
